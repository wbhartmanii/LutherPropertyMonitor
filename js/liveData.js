...
export async function tryFetchRiverStatsFromSources() {
        ...
        if (parsed?.points?.length) {
          ...
          if (stats) {
            return {
              ...
              stats,
              detail: `Live data pulled from ${source.name}`
            };
          }
        }
      } catch {
      ...
      if (parsed?.points?.length) {
        ...
        if (stats) {
          return {
            ...
            sourceUrl: source.siteUrl,
            stats,
            detail: `Live data parsed from ${source.name} page`
          };
        }
      }
    } catch {
  ...
  return {
    ok: false,
    detail:
      'Live river pull blocked or unparseable. Mock mode active. Add a serverless/API proxy for reliable production pulls.'
  };
}
